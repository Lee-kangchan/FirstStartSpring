        ...
        <div class="ct">
            <div class="ct_wrap">
                <div class="top_title">
                    ...
                    <h2><span class="title"><%=vo.getDescription() %></span></h2>
                </div>
                <div class="group_visual">
                    <div class="container_visual" style="width: 414px;">
                        <ul class="visual_img">
                            <li class="item" style="width: 414px;"> <img alt="" class="img_thumb" src="${pageContext.request.contextPath}/resources/<%=vo.getSave_file_name()%>"> <span class="img_bg"></span>
                                <div class="preview_txt">
                                    <h2 class="preview_txt_tit"></h2> <em class="preview_txt_dsc">₩<%=vo.getPrice()/1000 %>,000 ~ </em><em class="preview_txt_dsc">2017.2.17.(금)~2017.4.18.(화), 잔여티켓 2769매</em> </div>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="section_store_details">
                    <div class="store_details">
                        ...
                        <% break; } for(ProductVO vo: product){ %>
                        <p class="dsc">
                            	<%if(vo.getPrice_type_name().equals("A")){ %>성인(만 19~64세) <%=vo.getPrice()/1000 %>,000원 <br><%} %>
                            	<%if(vo.getPrice_type_name().equals("Y")){ %>청소년(만 13~18세) <%=vo.getPrice()/1000 %>,000원<br><%} %>
                            	<%if(vo.getPrice_type_name().equals("B")){ %>어린이(만 4~12세) <%=vo.getPrice()/1000 %>,000원 <br><%} %> 
                            	<%if(vo.getPrice_type_name().equals("S")){ %>20인 이상 단체 <%=vo.getPrice()/1000 %>,000원<br> <%} %> 
                            	<%if(vo.getPrice_type_name().equals("D")){ %>국가유공자, 장애인, 65세 이상 <%=vo.getPrice()/1000 %>,000원<br><%} %> 
                            	<%if(vo.getPrice_type_name().equals("C")){ %>지역주민 <%=vo.getPrice()/1000 %>,000원<br><%} %> 
                            	<%if(vo.getPrice_type_name().equals("E")){ %>어얼리버드 <%=vo.getPrice()/1000 %>,000원<br><%} %> 
                            	<%if(vo.getPrice_type_name().equals("R")){ %>R석 <%=vo.getPrice()/1000 %>,000원 <%} %> 
                            	<%if(vo.getPrice_type_name().equals("V")){ %>VIP <%=vo.getPrice()/1000 %>,000원 <%} %> 
                            	<%if(vo.getPrice_type_name().equals("D")){ %>평일 <%=vo.getPrice()/1000 %>,000원 <%} %> 
                            	<%System.out.println(vo.getPrice_type_name()); %>
                        </p>
                ...
                <form class="form_horizontal" name = "same">
                    <%for(ProductVO vo : product){ 
                    String R = vo.getPrice_type_name();
                    String who = (R.equals("A")) ? "어른" : (R.equals("Y")) ? "청소년" : (R.equals("B")) ? "어린이" : (R.equals("S")) ? "단체" : (R.equals("D")) ? "장애인" : (R.equals("C")) ? "지역주민" : (R.equals("E")) ? "어얼리버드" : (R.equals("R")) ? "R석" : (R.equals("V")) ? "VIP" : (R.equals("D")) ? "평일" : null;
                    %>
                        ...
                        <div class="qty">
                            <div class="count_control">
                                ...
                                <div class="clearfix">
                                    <a href="#" class="btn_plus_minus spr_book2 ico_minus3 disabled " title="빼기"> </a> <input type="tel" class="count_control_input disabled <%=vo.getId() %>" name = "num" value="0" readonly title="수량">
                                    <a href="#" class="btn_plus_minus spr_book2 ico_plus3 %>" title="더하기">
                                    </a>
                                </div>
                                <!-- [D] 금액이 0 이상이면 individual_price에 on_color 추가 -->
                                <div class="individual_price"><span class="total_price" id="<%=vo.getPrice()%>">0</span><span class="price_type">원</span></div>
                            </div>
                            
                            <div class="qty_info_icon"> <strong class="product_amount"> <span><%=who%></span> </strong> <strong class="product_price"> <span class="price"><%=vo.getPrice()/1000 %>,000원</span> <span class="price_type">원</span> </strong> <em class="product_dsc"><%=vo.getPrice()/1000 %>,000원 (<%=(int)vo.getDiscount_rate()%>% 할인가)</em> </div>
                        </div>      
                        <% } %>
                        ...
                        <div class="form_wrap">
                            ...
                            <form class="form_horizontal"  name="same" >
                                ...
                                <div class="inline_form"> <label class="label" for="tel"> <span class="spr_book ico_nessasary">필수</span> <span>연락처</span> </label>
                                	...
                                	<input type="hidden" value="<%for(ProductVO vo : product){%><%=vo.getId()%><%if(true)break;%><%}%>">
...
$(document).ready(function(){
	$('.ico_minus3').click(function(e){
		...
		if($(this).next().val() > 0)
		{
			...
			var price = parseInt($(this).parent().next().children().first().attr('id'));
			var amount = $(this).next().val();
			console.log($(".count_control_input").val());
			$(this).parent().next().children().first().empty();
			$(this).parent().next().children().first().text(price/1000*amount+',000');
		}
		if($(this).next().val() <= 0){

			($(this).next()).addClass("disabled");
			$(this).addClass("disabled"); 
			$(this).parent().next().removeClass("on_color");
			$(this).parent().next().children().first().empty();
			$(this).parent().next().children().first().text('0');
		}
		
		
	})
	$('.ico_plus3').click(function(e){
		e.preventDefault();
		e.stopPropagation();
		if($(this).prev().val() == 0){
			...
			$(this).parent().next().addClass("on_color");
		}
	...
	$("input[type=checkbox].chk_agree").change(function(){
